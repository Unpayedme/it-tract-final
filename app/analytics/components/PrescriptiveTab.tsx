...
const quadraticRegression = (values: number[]) => {
  ...
  const solve = (A: number[][], B: number[]) => {
    ...
    for(let i=0;i<size;i++){
      let maxRow=i;
      for(let k=i+1;k<size;k++) if(Math.abs(m[k][i])>Math.abs(m[maxRow][i])) maxRow=k;
...
const PrescriptiveTab: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
      {data.map(room => {
        if(!room || !room.bookingsOverTime || room.bookingsOverTime.length===0) return null;
        ...
        let recommendation="", optimalPrice = price;
        if(avgBookings<3){ recommendation="Low demand. Consider reducing price."; optimalPrice*=0.9; }
        else if(avgBookings<5){ recommendation="Stable demand. Maintain current pricing."; }
        else{ recommendation="High demand. Consider increasing price."; optimalPrice*=1.1; }

        const scatterData = dailyCounts.map((y,i)=>({x:i,y,date:room.bookingsOverTime[i].date}));

        let trendline: number[] = [];
        if(trendlineType==="linear") trendline = linearRegression(dailyCounts);
        else if(trendlineType==="quadratic") trendline = quadraticRegression(dailyCounts);
        ...
        let nextTrend = 0;
        if(trendlineType==="linear") nextTrend = linearRegression([...dailyCounts,0]).slice(-1)[0];
        else if(trendlineType==="quadratic") nextTrend = quadraticRegression([...dailyCounts,0]).slice(-1)[0];
        else nextTrend = cubicRegression([...dailyCounts,0]).slice(-1)[0];
        const trendlineData = trendline.map((y,i)=>({x:i,y})).concat({x:nextIndex, y:nextTrend});

        const r2 = calculateR2(dailyCounts, trendline);

        return (
          <div key={room.roomId} className="bg-gray-800 border border-gray-700 rounded-2xl p-6 shadow-lg flex flex-col justify-between hover:scale-[1.01] transition-all duration-300">
            <div className="grid grid-cols-1 gap-3">
              <MetricCard title="Current Price" value={`₱${price.toLocaleString()}`} color="bg-indigo-600"/>
            ...
            <div className="mt-6 bg-gray-900/60 backdrop-blur-sm p-5 rounded-xl border border-gray-700">
              <h3 className="text-lg font-semibold text-yellow-400">Recommendation 💡</h3>
            ...
            <div className="mt-6 bg-gray-900/50 p-4 rounded-xl border border-gray-700">
              <h3 className="text-lg font-semibold text-blue-300 mb-3">Demand Pattern Analysis</h3>
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm text-gray-300">Trendline:</label>
                <select value={trendlineType} onChange={e=>setTrendlineType(e.target.value as any)} className="bg-gray-700 text-white text-sm rounded px-2 py-1">
                  <option value="linear">Linear</option>
                  <option value="quadratic">Quadratic</option>
                  <option value="cubic">Cubic</option>
                </select>
                <span className="text-sm text-gray-400 font-mono ml-2">R² = {r2.toFixed(3)}</span>
              </div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ScatterChart margin={{top:10,right:20,bottom:60,left:60}}>
                  ...
                  <XAxis
                    ...
                    dataKey="x"
                    stroke="#9ca3af"
                    label={{ value: 'Date', position: 'insideBottom', offset: -45, fill:'#9ca3af' }}
                    tickFormatter={i => scatterData[i]?.date ? new Date(scatterData[i].date).toLocaleDateString('en-US',{month:'short',day:'numeric'}) : `Day ${i+1}`}
                  ...
                  <YAxis
                    ...
                    dataKey="y"
                    stroke="#9ca3af"
                    label={{ value: 'Number of Bookings', angle: -90, position: 'insideLeft', fill:'#9ca3af', offset:0 }}
                    allowDecimals={false}
                  />
                    ...
                    if(active && payload && payload[0]){
                      ...
                      return (
                        <div className="bg-gray-800 border border-gray-600 text-white p-3 rounded shadow-lg">
                          ...
                          <div className="text-sm">Date: {d.date ? new Date(d.date).toLocaleDateString('en-US',{weekday:'long', month:'short', day:'numeric'}) : `Forecast Day ${d.x+1}`}</div>
                          <div className="text-sm">Bookings: <span className="font-bold text-blue-400">{d.y}</span></div>
                        </div>
                      );
                    }
                    return null;
                  ...
                  <Scatter name="Daily Bookings" data={scatterData} fill="#3b82f6"/>
                  <Line type="linear" dataKey="y" data={trendlineData} stroke="#ef4444" strokeWidth={3} dot={false} strokeDasharray="5 5"/>
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          </div>
